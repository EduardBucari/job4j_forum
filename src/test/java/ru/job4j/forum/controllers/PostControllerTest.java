...
public class PostControllerTest {
    ...
    public void shouldPostServiceSaveCall() throws Exception {
        mockMvc.perform(
                        post("/post/save")
                                .param("caption","Куплю BMW. Дорого.")
                                .param("description","Телефон: 123456789")
                                .param("topicId","1")
                ).andDo(print())
                .andExpect(status().is3xxRedirection());
        ArgumentCaptor<Post> argument = ArgumentCaptor.forClass(Post.class);
        verify(posts, times(1)).save(argument.capture(), eq(0), eq(1));
        assertThat(argument.getValue().getCaption(), is("Куплю BMW. Дорого."));
        assertThat(argument.getValue().getDescription(), is("Телефон: 123456789"));
    }
}